...
// Revision: Dec 18 - 1996
// Fixes Needed:
// ESC Quit (Are You Sure?)
// Save over Existing file (Y/n)
// Load Old and Save Old -> New
//
// DRAW V 6
// This is the only version I found so far, it has the undocumented ] load
// old most of this version works with small bugs and the features listed
// above.
// directory c:\borlandc\work96\draw7
// Edited: July 15, 2001
// By Franco
// Note: Turn Doesn't work, neither does Undo
//
// File Edit: Januart 23rd , 2003
...
long int vc = 36109;		// Upper Left hand corner of MOve Object for Erase
long int oc = 36430;		// Original Coordinate y *320 + x
long int toc = 36430;		// Backup of Original Coordinate y *320 + x
...
void PutColor()
{
	...
	for (j=1;j<256;j++)
	{
		...
		for (y=0;y<4;y++)
		{
		  *(screen+adv+y+ad+offset+n+321)=j;
		}
...

{
	...
	int gx,gy,cx,cy,incr=0,dninc=0;
