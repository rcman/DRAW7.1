...
int fileread2(void);
void ClearData(void);
void arrowmoveLR(int LR);
void arrowmoveUD(int UD);
...
//      if (!Get_Mouse(1))
 ...
 //     printf("Sprite %d  Row %d ",sp,rw);
	...
	if (hc == 1 && lc == 27)   // ESC Key & Exit
		...
		if (hc == 77 && lc == 0)        // -> Right Arrow Move
		{				//
		arrowmoveLR(1);
		placeshape(rw,sp);
		nextshape(rw,sp);
...
void arrowmoveLR(int LR)
{
	...
	for (i=0;i<16;i++)
		...
		if (sel==1)
		   {
		   a=1;
		   animobjects[rw].fshp[sp]->shp[i*16+j]=animobjects[rw].fshp[sp]->shp[a*16*b];
		   a+=1;
